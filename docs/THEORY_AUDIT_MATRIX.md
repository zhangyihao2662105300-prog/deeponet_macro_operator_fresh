...
| T15 | Macro16 source128 recovery force closes. | Assemble recovery force from `B_macro_qraw`, stress, and an explicit physical-volume mode. | Mean and max force errors below gate thresholds. | Current canonical volume decision is selected-frame physical volume in `docs/GATE_DECISIONS.md`; `reports/03a_volume_convention_audit.md` shows `case031` force rel drops to about `0.0086`, but the full Gate 03 audit script has not been rerun with this explicit mode. | 待验证 |
