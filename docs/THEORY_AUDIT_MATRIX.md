...
| T06 | Valid geometries require positive Jacobian determinant. | Check all integration-point Jacobian determinants for each geometry. | No negative or near-zero determinant. | `reports/04_distortion_generality_audit.md` verifies positive detJ for audited regular, light, medium, cylindrical, conical, thickness-varying, mild double-curvature, and repaired strong non-inverted geometries. | 已验证 |
...
| T15 | Macro16 source128 recovery force closes. | Assemble recovery force from `B_macro_qraw`, stress, and an explicit physical-volume mode. | Mean and max force errors below gate thresholds. | `reports/03_macro16_force_stiffness_reaudit.md` reruns all 10 compacts with `--volume-mode selected-frame`; force mean rel is `0.0012727832304676163` and max rel is `0.00858572515082126`. | 已验证 |
| T16 | Macro16 source128 full tangent stiffness closes. | Compare an explicitly named full tangent candidate against perturbed-force finite-difference stiffness. | Mean and max full tangent errors below gate thresholds. | `reports/03_macro16_force_stiffness_reaudit.md` shows material-only `B^T D B dV` remains a diagnostic with max rel `0.031962110431098374`, worst `case031`; `full-fd-reference` closes by construction but is not a Macro16 full tangent candidate. | 待验证 |
| T17 | Macro16 material-only stiffness is symmetric for the current material/assembly path. | Measure macro material-only stiffness symmetry relative error. | Symmetry relative error below threshold. | `reports/03_macro16_force_stiffness_reaudit.md` max material-only stiffness symmetry rel `1.3549217800954415e-16`. | 已验证 |
| T18 | The active volume convention for large-response Abaqus RF audit is fixed. | Compare reference/standard, selected-frame Abaqus IVOL, and inferred-volume assembly on the Gate 03 blocker. | The project selects one convention and records what physical response it is meant to close against. | `docs/GATE_DECISIONS.md` adopts selected-frame physical volume as canonical audit volume and keeps `integration_weight_phys` as reference/standard volume. | 已验证 |
| T19 | Material-only stiffness `B^T D B dV` is an approximate audit stiffness, not the complete Abaqus tangent under large deformation. | Compare material-only stiffness with perturbed-force finite-difference tangent under large-response cases. | Tangent error below Gate 03 thresholds, or the missing terms are explicitly added. | `reports/03_macro16_force_stiffness_reaudit.md` reports selected-frame material-only stiffness mean rel `0.0036642982829359003` and max rel `0.031962110431098374`; this remains a diagnostic residual, not full tangent closure. | 近似 |
| T20 | Consistent tangent terms required by Gate 03 are included. | Audit or derive contributions from `dV/dq`, `dB/dq`, and geometric/stress stiffness when comparing against Abaqus finite-difference tangent. | Gate 03 stiffness max error below `0.02` on the full case set. | `reports/03_macro16_force_stiffness_reaudit.md` verifies the `full-fd-reference` branch with mean/max rel `0.0` by construction, but no Macro16 full consistent tangent candidate is implemented. | 待验证 |
...
| T24 | Generality holds beyond the current 10-case set. | Repeat data and mechanics gates on regular, light, medium, cylinder, cone, thickness variation, and mild double-curvature shells. Keep strong non-inverted distortion as a non-blocking robustness boundary test. | Required wind-turbine shell geometry families pass force closure and selected material diagnostics are recorded. | `reports/04_distortion_generality_audit.md` shows regular, light, medium, cylindrical, conical, thickness-varying, and mild double-curvature selected-frame force closure pass. Repaired strong non-inverted robustness data no longer duplicates `case060` and passes selected-frame effective force closure. | 已验证 |
| T25 | The current 96/64/32 integration-point reduction candidates can replace the active 128-point rule. | Audit 128, 96, 64, 32, and 18-point rules with selected-frame force closure. | Candidate rule force max relative error below `0.02`; material-only K is diagnostic only. | `reports/05_ip_reduction_audit.md` shows 128 passes with force mean/max `0.0004799514766587757` / `0.00858572515082126`; 96, 64, and 32 fail with force max `0.32625651778316855`, `0.6331713463076637`, and `0.9948859290594079`; 18 remains a failed control with max `0.6609305859267052`. | 待决策 |
