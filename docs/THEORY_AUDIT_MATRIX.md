...
| T06 | Valid geometries require positive Jacobian determinant. | Check all integration-point Jacobian determinants for each geometry. | No negative or near-zero determinant. | `reports/04_distortion_generality_audit.md` verifies positive detJ for audited regular, light, medium, cylindrical, conical, thickness-varying, and mild double-curvature geometries; true strong non-inverted geometry remains a non-blocking robustness item. | 已验证 |
