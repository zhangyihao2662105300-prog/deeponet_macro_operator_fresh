...
| The active physical-volume convention for large-response force assembly is selected before closing Gate 03. | 待决策 | `reports/macro16_case031_mechanics_diagnosis.md` shows selected-frame or inferred volume reduces `case031` force rel from about `0.036` to about `0.0086`, while reference/standard weights remain internally consistent. |
...
| Physical assembly must use `integration_weight_phys`. | 已验证 | `reports/macro16_force_stiffness_audit.md` |
