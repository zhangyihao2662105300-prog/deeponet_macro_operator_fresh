...
The preferred compact schema is to store macro-node geometry and point features
directly:

```text
X_macro / macro_node_coords / nodes50: [N,50,3] or [50,3]
...
`X_macro` and point fields such as `ip_keys`/`ip_xyz`, but the original
...
```text
X_macro  50 reference nodes in CSS8 node-id order
