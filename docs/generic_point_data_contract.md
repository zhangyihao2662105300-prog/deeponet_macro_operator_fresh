...
and `run_true176_shape4_template_plus_smoke.py`, the compact files dropped
`ip_keys` and `ip_xyz`, but the original generation contract is deterministic:

```text
shape4 -> 50 CSS8 nodes -> 16 CSS8 elements -> 128 Gauss rows
```
...
That means the reference point fields can be reconstructed from `shape4`:
