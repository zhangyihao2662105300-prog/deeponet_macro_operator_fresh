...
from build_macro16_source128_teacher import build_all as build_macro16_source128_teacher
...
def test_macro16_fixed128_state_b_model_has_static_b_and_ad_matches_when_detached() -> None:
    point_table = macro16_source128_point_table()
    x16 = macro16_flat_x16()
    point, _names, _fields = Macro16GeometryMap(x16).build_point_features(point_table)
    p_norm = torch.as_tensor(np.broadcast_to(point.reshape(1, *point.shape), (2, *point.shape)).copy(), dtype=torch.float32)
    x_norm = torch.randn(2, 97)
    x_norm[:, :48] = x_norm[:, :48] * 0.1
    q0 = torch.randn(48) * 0.05
    skip = torch.zeros(128, 6, 48)
    skip[:, 0, 0] = 0.2
    skip[:, 1, 1] = -0.1
    ...
    out = model(x_norm, p_norm)
    assert out.shape == (2, 128, 6)
    assert tuple(model.static_b_norm.shape) == (128, 6, 48)
    explicit = model._state_b_norm(x_norm, p_norm)
    j = ad_jacobian(model, x_norm, p_norm, [0, 1, 2], create_graph=False, method="forward")
    assert torch.allclose(j, explicit[:, :, :, :3], atol=1.0e-5, rtol=1.0e-5)
    assert model.state_b_config()["state_b_kind"] == "fixed128_point_q_rank"
    try:
        model(x_norm, p_norm[:, :127])
    except ValueError as exc:
        assert "128" in str(exc)
    else:
        raise AssertionError("fixed128 state B accepted non-128 point input")


def test_macro16_fixed128_direct_state_b_loss_uses_explicit_physical_b() -> None:
    point_table = macro16_source128_point_table()
    x16 = macro16_flat_x16()
    point, _names, _fields = Macro16GeometryMap(x16).build_point_features(point_table)
    p_norm = torch.as_tensor(np.broadcast_to(point.reshape(1, *point.shape), (2, *point.shape)).copy(), dtype=torch.float32)
    ...
    model = Macro16BoundaryDeepONetWithLE0Fixed128StateB(
        ...
        state_b_rank=3,
        detach_state_b=True,
        state_b_zero_init=True,
    )
    columns = [0, 1, 2]
    le_std = torch.ones(1, 128, 6, 1)
    q_std = torch.ones(len(columns))
    target = model._state_b_norm(x_norm, p_norm).detach()[:, :, :, columns]
    b_scale = torch.ones(1, 1, 6, len(columns))
    loss0 = explicit_state_b_physical_balanced_loss(
        model,
        x_norm,
        p_norm,
        target,
    ...
    loss1 = explicit_state_b_physical_balanced_loss(
        model,
        x_norm,
        p_norm,
        target + 0.1,
...
def test_macro16_fixed128_state_b_training_checkpoint_loads_for_force_audit() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        compact = root / "case001_macro16_fixed128_state_b.npz"
        out_dir = root / "out"
        n = 4
        x16 = macro16_flat_x16()
        ...
        q48 = rng.normal(scale=0.05, size=(n, 48)).astype(np.float32)
        point_count = 128
        b = np.zeros((n, point_count, 6, 48), dtype=np.float32)
        b[:, :, 0, 0] = 0.2
        b[:, :, 1, 1] = -0.1
        b[:, :, 2, 2] = 0.05 + 0.25 * q48[:, None, 0]
        le = np.einsum("npaj,nj->npa", b, q48).astype(np.float32)
        ...
        args = SimpleNamespace(
            ...
            trunk_depth=2,
            activation="tanh",
            model_style="le0-fixed128-state-b",
            residual_scale=0.0,
            le0_scale=0.0,
            freeze_le0_static=False,
            freeze_le0_point=True,
        ...
        assert isinstance(loaded, Macro16BoundaryDeepONetWithLE0Fixed128StateB)
        branch_mean = np.asarray(checkpoint["norms"]["branch_mean"], dtype=np.float32).reshape(-1)
        point_mean = np.asarray(checkpoint["norms"]["point_mean"], dtype=np.float32).reshape(-1)
        xb = torch.zeros(1, branch_mean.size)
...
def test_macro16_force_stiffness_audit_reports_source128_candidate() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        source = root / "source128_force.npz"
        compact = root / "macro16_from_source128.npz"
        out = root / "force128_audit.json"
        n = 1
        p_macro = 2
        p_source = 128
        x16 = macro16_flat_x16()
        q48 = np.zeros((n, 48), dtype=np.float32)
        elastic_d = np.diag([3.0, 4.0, 5.0, 6.0, 7.0, 8.0]).astype(np.float64)
        le128 = np.zeros((n, p_source, 6), dtype=np.float64)
        le128[:, :, 0] = np.linspace(0.1, 0.4, p_source).reshape(1, p_source)
        le128[:, :, 2] = np.linspace(-0.2, 0.2, p_source).reshape(1, p_source)
        b128 = np.zeros((n, p_source, 6, 48), dtype=np.float64)
        b128[:, :, 0, 0] = 0.8
        b128[:, :, 2, 7] = np.linspace(0.2, 0.5, p_source).reshape(1, p_source)
        ivol = np.full((1, p_source), 0.01 / p_source, dtype=np.float64)
        stress128 = np.einsum("ab,nrb->nra", elastic_d, le128)
        force_macro_order = np.einsum("nraj,nra,nr->nj", b128, stress128, np.broadcast_to(ivol, (n, p_source)))
        stiffness_macro_order = np.einsum("nraj,ab,nrbk,nr->njk", b128, elastic_d, b128, np.broadcast_to(ivol, (n, p_source)))
        source_to_macro = np.empty(48, dtype=np.int64)
        source_to_macro[TRUE176_MACRO_TO_KEEP_FLAT] = np.arange(48, dtype=np.int64)
        force_source_order = np.zeros_like(force_macro_order)
        force_source_order[:, TRUE176_MACRO_TO_KEEP_FLAT] = force_macro_order
        delta = 1.0e-6
        directions_source = TRUE176_MACRO_TO_KEEP_FLAT.copy()
        plus_source = np.empty((n, 48, 48), dtype=np.float64)
        for pos, source_dir in enumerate(directions_source.tolist()):
            macro_dir = int(source_to_macro[source_dir])
            plus_macro = force_macro_order + delta * stiffness_macro_order[:, :, macro_dir]
            plus_source[:, pos, TRUE176_MACRO_TO_KEEP_FLAT] = plus_macro
        le_macro_bad = np.zeros((n, p_macro, 6), dtype=np.float64)
        b_macro_bad = np.zeros((n, p_macro, 6, 48), dtype=np.float64)
        weights_bad = np.ones((n, p_macro), dtype=np.float64)
        np.savez(
            source,
            RF_projected=force_source_order,
            RF_projected_plus=plus_source,
            perturb_directions=directions_source,
            delta=np.asarray(delta, dtype=np.float64),
            elastic_D=elastic_d,
            ...
            B_LE128_forward=b128,
            ip_IVOL_abaqus=ivol,
            IVOL128_inferred_from_DLE=np.broadcast_to(ivol, (n, p_source)),
        )
        np.savez_compressed(
            compact,
            q48_raw=q48,
            X16=np.broadcast_to(x16.reshape(1, 16, 3), (n, 16, 3)).astype(np.float32),
            LE_macro=le_macro_bad.astype(np.float32),
            B_macro=b_macro_bad.astype(np.float32),
            integration_weight_hat=weights_bad.astype(np.float32),
            source_compact=np.asarray(str(source), dtype=object),
            source_row=np.arange(n, dtype=np.int64),
            source_node_order=np.asarray("true176-keep", dtype=object),
            strain_output_coordinate=np.asarray("macro16_local_frame", dtype=object),
        ...
        summary = run_macro16_force_stiffness_audit(args)
        row = summary["compacts"][0]
        assert row["force"]["rel"] > 0.9
        source128 = row["candidates"]["source128_ip_volume"]
        assert source128["available"] is True
        assert source128["force"]["rel"] < 1.0e-6
        assert source128["stiffness"]["rel"] < 1.0e-6
...
def test_macro16_source128_teacher_builder_writes_force_closing_compact() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        source = root / "complete_case007_v2b_local_strain.npz"
        out_root = root / "source128"
        audit_out = root / "source128_force_audit.json"
        n = 2
        p = 128
        x16 = macro16_flat_x16()
        q48 = np.zeros((n, 48), dtype=np.float32)
        q48[1, 0] = 0.2
        elastic_d = np.diag([4.0, 5.0, 6.0, 7.0, 8.0, 9.0]).astype(np.float64)
        le128 = np.zeros((n, p, 6), dtype=np.float64)
        le128[:, :, 0] = np.linspace(0.1, 0.3, p).reshape(1, p)
        le128[:, :, 1] = np.asarray([0.2, 0.25], dtype=np.float64).reshape(n, 1)
        b128_macro = np.zeros((n, p, 6, 48), dtype=np.float64)
        b128_macro[:, :, 0, 0] = 1.5
        ...
        for pos, source_dir in enumerate(directions_source.tolist()):
            ...
            plus_source[:, pos, TRUE176_MACRO_TO_KEEP_FLAT] = plus_macro
        true176_keep_to_macro = np.asarray([0, 1, 2, 7, 3, 6, 5, 4, 8, 9, 10, 15, 11, 14, 13, 12], dtype=np.int64)
        x_keep = x16[true176_keep_to_macro]
        q_keep = np.zeros_like(q48)
        q_keep[:, TRUE176_MACRO_TO_KEEP_FLAT] = q48
        np.savez(
            source,
            q48_raw=q_keep,
            X_keep=x_keep,
            LE128_base=le128.astype(np.float32),
            B_LE128_forward=b128_source.astype(np.float32),
            RF_projected=force_source,
            RF_projected_plus=plus_source,
            perturb_directions=directions_source,
            delta=np.asarray(delta, dtype=np.float64),
            elastic_D=elastic_d,
            ip_IVOL_abaqus=ivol,
            LE128_local=le128.astype(np.float32),
            T_eps_from_abq=np.broadcast_to(np.eye(6, dtype=np.float64).reshape(1, 6, 6), (128, 6, 6)),
            ip_keys=np.asarray([[elem, ip, 0] for elem in range(1, 17) for ip in range(1, 9)], dtype=np.int64),
            case_id=np.asarray([7, 7], dtype=np.int64),
            strain_field=np.asarray("LE", dtype=object),
            B_label_strain_field=np.asarray("LE", dtype=object),
        )
        build_args = SimpleNamespace(
        ...
        build_summary = build_macro16_source128_teacher(build_args)
        compact = Path(build_summary["cases"][0]["macro16_compact"])
        with np.load(compact, allow_pickle=True) as z:
        ...
        audit = run_macro16_force_stiffness_audit(audit_args)
        assert audit["strict_pass"] is True
        row = audit["compacts"][0]
        assert row["point_count"] == 128
